...
Then(/^the "(.*)" should look like$/) do |table_id, definition_table|
  column_headers = []
  @browser.table(:id,table_id)[1].each { |h| column_headers << h.text.strip }
  ...
  definition_table.hashes.each_with_index do |definition_table_row, row_index|
    ...
    unless @browser.element_by_xpath("//table[@id='#{table_id}']/tbody").nil?
      html_row = @browser.elements_by_xpath("//table[@id='#{table_id}']/tbody/tr[#{row_index + 1}]/td")
    else
      html_row = @browser.elements_by_xpath("//table[@id='#{table_id}']//tr[#{row_index + 2}]/td")
    end
