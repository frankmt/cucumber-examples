...
module Selenium
    ...
    class SeleniumDriver
        ...
        def parse_number(number)
          return number =~ /\./ ? number.to_f : number.to_i
        end
        # TW PATCH ---------------------------------------
        
        def get_number(verb, args)
            # TW PATCH ---------------------------------------
            return parse_number(get_string(verb, args))
            # TW PATCH ---------------------------------------
        end
        
        def get_number_array(verb, args)
            # TW PATCH ---------------------------------------
            return get_string_array(verb, args).map {|n| parse_number(n)}
            # TW PATCH ---------------------------------------
        end
        ...
        # TW Patch --------------------------------------
        def click_and_wait(link, timeout=20_000)
          self.click link
        ...
        def wait_for_element_present(locator)
          do_command("waitForElementPresent", [locator,])
        end
        # TW PATCH
